...
test.describe('Home/End Navigation', () => {
  ...
  test('[GRID.5-E2E-001] @p0 Home moves to first column in current row', async ({ gridPage }) => {
    // GIVEN: User has selected a cell in the middle of a row (Bench Press, third week — index 3)
  ...
  test('[GRID.5-E2E-002] @p0 End moves to last column in current row', async ({ gridPage }) => {
    // GIVEN: User has selected a cell in the first column (Bench Press, exercise column — index 3)
    ...
    await gridPage.expectActiveCellAt(3, WEEKS_COUNT)
  })

  test('[GRID.5-E2E-003] @p1 Ctrl+Home moves to first cell (top-left)', async ({ gridPage }) => {
    // GIVEN: User has selected a cell in the middle of the grid (Tricep Pushdown, third week — index 5)
