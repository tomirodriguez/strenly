...
function FieldWarning({
  ...
  const content = useMemo(() => {
    ...
    const uniqueWarnings = [...new Map(warnings.map((warning) => [warning?.message, warning])).values()]

    if (uniqueWarnings?.length === 1) {
      return uniqueWarnings[0]?.message
    }

    return (
      <ul className="ml-4 flex list-disc flex-col gap-1">
        {uniqueWarnings.map((warning, index) => warning?.message && <li key={index}>{warning.message}</li>)}
      </ul>
    )
  }, [children, warnings])
  ...
  return (
    <div
      ...
      className={cn('font-normal text-amber-600 text-sm dark:text-amber-500', className)}
      {...props}
    >
      {content}
    </div>
