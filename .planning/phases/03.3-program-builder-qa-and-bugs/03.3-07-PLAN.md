...
**Update use-save-draft.ts mutation hook:**

Add new fields to the mutation call:

```typescript
// In apps/coach-web/src/features/programs/hooks/mutations/use-save-draft.ts
export function useSaveDraft(programId: string) {
  const queryClient = useQueryClient()
  const { getChanges, markSaved } = useGridActions()
...
Update the save button to pass structural changes:

```typescript
// In save-button.tsx or wherever the save is triggered
const handleSave = () => {
  const changes = getChanges()
  saveDraft.mutate({
    programId,
    prescriptions: changes.prescriptions.map(/* ... */),
    exerciseRows: changes.exerciseRows.map(/* ... */),
    groups: [],  // TODO: group changes if implemented
