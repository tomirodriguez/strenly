...
<task type="auto">
  <name>Task 2: Verify sessionRows contains superset group data</name>
  <files>
apps/coach-web/src/components/programs/program-grid/exercise-row.tsx
...
If any of these are missing, add them.
  ...
  <verify>
1. Add console.log in ExerciseRowActions to verify sessionRows contains group data:
   console.log('sessionRows:', sessionRows)
2. Open program grid with existing superset exercises
3. Verify console shows sessionRows with groupId values
4. Remove console.log after verification
  </verify>
  <done>Data flow verified: sessionRows with groupId reaches ExerciseRowActions.</done>
</task>
