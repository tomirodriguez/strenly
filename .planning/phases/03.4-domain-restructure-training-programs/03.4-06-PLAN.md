...
**Verified:** This function exists at `packages/contracts/src/programs/prescription.ts`
</series_formatting>
...
# Existing formatter - USE THIS (verified to exist)
...
<task type="auto">
  ...
  <action>
    ...
    2. Import and use formatSeriesToNotation from @strenly/contracts/programs/prescription

    **IMPORTANT:** Verify the import works before using:
    ```typescript
    // This function exists - verified at packages/contracts/src/programs/prescription.ts
    import { formatSeriesToNotation } from '@strenly/contracts/programs/prescription'
    ```
    ...
    export function transformProgramToGrid(
      ...
      // Build rows from sessions (use first week as canonical structure)
      // For each exercise item:
      //   - Look up exerciseName via exercisesMap.get(item.exerciseId)
      //   - Format series via formatSeriesToNotation(item.series)
      // Return { columns, rows }
    }
    ```

    Key details:
    - Use exercisesMap.get(item.exerciseId) ?? 'Ejercicio desconocido' for exercise names
    - Use formatSeriesToNotation(weekItem.series) for prescription notation
    - Handle superset grouping with groupLetter (A, B, C...) and groupIndex (1, 2, 3...)
  </action>
  <verify>
    ```bash
    # Verify formatSeriesToNotation import works
    cd /Users/tomiardz/Projects/treino && grep -q "formatSeriesToNotation" packages/contracts/src/programs/prescription.ts && echo "Function exists in contracts"

    # Verify it's imported in transform file after changes
    grep -q "import.*formatSeriesToNotation.*from.*@strenly/contracts" apps/coach-web/src/components/programs/program-grid/transform-program.ts && echo "Import added"
...
# Verify formatSeriesToNotation is imported from contracts
...
- Transform function imports and uses formatSeriesToNotation from @strenly/contracts
