...
must_haves:
  ...
  artifacts:
    ...
    - path: "packages/core/src/ports/program-repository.port.ts"
      provides: "Port interface for repositionRowToAfterSupersetGroup method"
      contains: "repositionRowToAfterSupersetGroup"
    - path: "packages/backend/src/infrastructure/repositories/program.repository.ts"
      provides: "Repository implementation of row repositioning"
      contains: "repositionRowToAfterSupersetGroup"
  ...
  key_links:
    ...
    - from: "toggle-superset.ts"
      to: "program.repository.ts"
      via: "repositionRowToAfterSupersetGroup call"
      pattern: "repositionRowToAfterSupersetGroup"
---
...
<task type="auto">
  ...
  <skills>/port, /repository, /use-case</skills>
  <files>
    packages/core/src/ports/program-repository.port.ts
    packages/backend/src/infrastructure/repositories/program.repository.ts
    packages/backend/src/use-cases/programs/toggle-superset.ts
...
**STEP 1: Add method signature to ProgramRepositoryPort (port FIRST per Clean Architecture):**

In `packages/core/src/ports/program-repository.port.ts`, add to the interface:
...
**STEP 3: Call from toggle-superset.ts:**
