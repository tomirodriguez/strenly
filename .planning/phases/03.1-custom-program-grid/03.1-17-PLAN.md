...
<task type="auto">
  <name>Task 2: Add findExerciseRowsBySessionId to port and repository</name>
  <skills>/repository</skills>
  <files>
packages/core/src/ports/program-repository.port.ts
packages/backend/src/infrastructure/repositories/program.repository.ts
...
<task type="auto">
  <name>Task 3: Add repositioning on superset removal</name>
...
<task type="auto">
  <name>Task 4: Add superset adjacency validation to reorder</name>
  <skills>/use-case</skills>
...
Note: This task depends on Task 2 which creates the `findExerciseRowsBySessionId` repository method.
  </action>
  <verify>
1. `pnpm typecheck` passes
2. Manual test:
