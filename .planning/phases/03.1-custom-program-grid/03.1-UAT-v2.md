...
- truth: "Athlete selector has keyboard accessibility - arrow keys navigate search results"
  ...
  severity: major
  test: 1
  root_cause: "Custom Popover-based implementation (lines 105-225) uses raw HTML elements with no keyboard handlers. Project already has accessible Combobox component from @base-ui/react that should be used instead."
  artifacts:
    - path: "apps/coach-web/src/features/programs/components/program-form.tsx"
      issue: "Lines 105-225: Custom implementation with Popover + raw input/buttons, no onKeyDown handlers"
    - path: "apps/coach-web/src/components/ui/combobox.tsx"
      issue: "Existing accessible Combobox component not being used"
  missing:
    - "Replace custom Popover implementation with Combobox component"
    - "Use ComboboxInput with controlled value for search"
    - "Use ComboboxItem for athlete options"
  debug_session: ".planning/debug/athlete-selector-a11y.md"
...
- truth: "Superset menu shows existing groups to join, and creating a superset combines exercises physically in the grid"
  ...
  severity: major
  test: 3
  root_cause: "toggle-superset.ts ONLY updates supersetGroup/supersetOrder metadata - it does NOT reposition rows. Design gap: use case was designed to 'tag' rows, not physically reorder them. Menu logic is correct but user tested on fresh session without existing supersets."
  artifacts:
    - path: "packages/backend/src/use-cases/programs/toggle-superset.ts"
      issue: "Lines 73-108: Only updates metadata, no repositioning logic"
  missing:
    - "After assigning superset group, calculate new orderIndex to place row adjacent to group members"
    - "Call reorderExerciseRows to update all row positions"
  debug_session: ".planning/debug/superset-functionality-issues.md"
...
- truth: "Adding exercise to existing superset group assigns next order number and rearranges rows to keep group adjacent"
  ...
  test: 4
  related_to: 3
  root_cause: "reorder-exercise-rows.ts and repository do NOT recalculate supersetOrder after row repositioning. supersetOrder is set ONCE when joining superset and NEVER updated thereafter."
  artifacts:
    - path: "packages/backend/src/use-cases/programs/reorder-exercise-rows.ts"
      issue: "Lines 19-44: Missing supersetOrder recalculation"
    - path: "packages/backend/src/infrastructure/repositories/program.repository.ts"
      issue: "Lines 1007-1041: reorderExerciseRows only updates orderIndex, not supersetOrder"
  missing:
    - "After updating orderIndex values, recalculate supersetOrder for each group based on new physical order"
    - "Alternative: Remove supersetOrder storage, calculate dynamically in transform-program.ts"
  debug_session: ".planning/debug/superset-functionality-issues.md"
