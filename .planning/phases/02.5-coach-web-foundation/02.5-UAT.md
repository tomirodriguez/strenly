...
result: issue
reported: "No esta bien hecho. Deberia ser un solo item 'Invitar' que despliegue el modal. Si ya tiene invitacion, mostrarla y permitir copiar. Si no tiene, boton para crearla. Regenerar esta bien. UI: modal max-width hace que link salga por fuera. Link deberia truncarse con ... para caber dentro."
severity: minor
...
- truth: "Invitation modal UX: single 'Invitar' action, truncated URL"
  ...
  artifacts:
    - path: "apps/coach-web/src/features/athletes/components/invitation-modal.tsx"
      issue: "URL not truncated, overflows modal max-width"
    - path: "apps/coach-web/src/features/athletes/components/athletes-table.tsx"
      issue: "Separate 'Generar invitación' and 'Ver invitación' actions instead of unified 'Invitar'"
  missing:
    - "Consolidate to single 'Invitar' row action"
    - "Modal shows create button if no invitation, invitation details if exists"
    - "Truncate invitation URL with ellipsis (text-ellipsis overflow-hidden)"
  debug_session: ""
