...
**Primary recommendation:** Use immer + useReducer for client-side grid state, store series as JSONB array with ordered index, implement exercise groups table for unified grouping model.
...
|---------|---------|---------|--------------|
| immer | 10.x | Immutable state updates | Industry standard for complex nested state; produces next immutable state via "mutations" on draft |
| use-immer | 0.10.x | React hooks for immer | Official React bindings, provides useImmerReducer |
...
|------------|-----------|----------|
| immer + useReducer | zustand | Zustand is external store; local component state is simpler for isolated grid |
| immer + useReducer | useState with spread | Spread syntax for deeply nested updates is error-prone and verbose |
| JSONB array | Normalized series table | Normalized table adds complexity; JSONB keeps prescription atomic |

**Installation:**
```bash
pnpm add immer use-immer  # Likely already installed; verify
```
...
### Pattern 3: Client-Side Grid State with useImmerReducer

**What:** All grid edits happen locally; single mutation saves all changes.

**When to use:** Program grid editing with "Guardar" button.

**Structure:**
```typescript
// Source: https://github.com/immerjs/use-immer, https://react.dev/reference/react/useReducer

import { useImmerReducer } from 'use-immer'

// Grid state holds the full editable program
interface GridState {
...
type GridAction =
  | { type: 'UPDATE_PRESCRIPTION'; groupId: string; exerciseOrder: number; weekId: string; series: PrescriptionSeries[] }
  | { type: 'UPDATE_EXERCISE'; groupId: string; exerciseOrder: number; exerciseId: string }
  | { type: 'ADD_EXERCISE_TO_GROUP'; groupId: string; exerciseId: string }
  | { type: 'REMOVE_EXERCISE'; groupId: string; exerciseOrder: number }
  | { type: 'CREATE_GROUP'; sessionId: string; exerciseId: string }
  | { type: 'MERGE_INTO_GROUP'; sourceGroupId: string; targetGroupId: string }
  | { type: 'SPLIT_FROM_GROUP'; groupId: string; exerciseOrder: number }
  | { type: 'REORDER_GROUPS'; sessionId: string; groupIds: string[] }
  | { type: 'REORDER_WITHIN_GROUP'; groupId: string; exerciseOrders: number[] }
  | { type: 'RESET'; program: ProgramWithDetails }  // Reset to server state
  | { type: 'MARK_SAVED' }  // Clear dirty flag

function gridReducer(draft: GridState, action: GridAction) {
  // Immer allows direct mutations on draft
  switch (action.type) {
    case 'UPDATE_PRESCRIPTION':
      // Find and update the series array
      draft.isDirty = true
      break
    // ... other cases
  }
...
### Example 2: Grid State Reducer with Immer

```typescript
// Source: https://github.com/immerjs/use-immer pattern
...
type GridAction =
  | { type: 'UPDATE_SERIES'; exerciseId: string; weekId: string; series: SeriesInput[] }
  | { type: 'ADD_TO_GROUP'; groupId: string; exerciseId: string }
  | { type: 'RESET'; program: ProgramWithDetails }
  | { type: 'MARK_SAVED' }

function gridReducer(draft: GridState, action: GridAction) {
  switch (action.type) {
    case 'UPDATE_SERIES': {
      // Find the prescription and update its series
      for (const session of draft.program.sessions) {
        for (const row of session.rows) {
          if (row.exerciseId === action.exerciseId) {
            const prescription = row.prescriptionsByWeekId[action.weekId]
            if (prescription) {
              prescription.series = action.series
            } else {
              // Create new prescription
              row.prescriptionsByWeekId[action.weekId] = {
                id: `rx-temp-${Date.now()}`,
...
export function useGridState(initialProgram: ProgramWithDetails) {
