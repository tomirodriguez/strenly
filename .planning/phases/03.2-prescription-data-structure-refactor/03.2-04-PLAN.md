...
must_haves:
  truths:
    ...
    - "Repository port has saveDraft method for bulk saves"
...
- Updated repository port interface (including saveDraft for Plan 05)
...
// Bulk save method (required by Plan 05 saveDraft use case)
saveDraft(
  ctx: OrganizationContext,
  input: SaveDraftInput
): ResultAsync<{ updatedAt: Date }, RepositoryError>
```
...
- Implement saveDraft: bulk update using transaction for atomicity
  - Update prescriptions with series arrays
  - Update exercise row selections
  - Update group memberships
  - Update program.updatedAt timestamp
...
Keep existing superset-based methods working for now - they'll be deprecated later.
  ...
  <done>Repository port has group methods and saveDraft, implementation queries exercise_groups table</done>
</task>
...
- [ ] Repository port has saveDraft method (required by Plan 05)
- [ ] Repository implementation handles exercise_groups table
- [ ] Repository implementation has saveDraft with transaction
