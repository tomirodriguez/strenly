...
must_haves:
  truths:
    ...
    - "Existing per-change mutations still work"
  artifacts:
...
Then update the repository port to add the saveDraft method that handles the bulk operation:
- Update prescriptions with series arrays
- Update exercise row selections
- Update group memberships

The repository should use a transaction to ensure atomicity.
  </action>
...
<task type="auto">
  <name>Task 3: Create saveDraft procedure and wire up</name>
...
3. Update the repository implementation with saveDraft method that:
- Uses a database transaction
- Updates prescriptions.series JSONB for each prescription change
- Updates program_exercises.exerciseId for row changes
- Updates group memberships (exerciseGroups table and program_exercises.groupId)
- Updates program.updatedAt timestamp
  </action>
  <verify>pnpm typecheck --filter @strenly/backend</verify>
  <done>saveDraft procedure exists and is wired into programs router</done>
</task>
...
- [ ] Repository saveDraft method handles bulk updates
- [ ] All packages typecheck
- [ ] Existing per-change mutations still work (not removed)
</success_criteria>
