...
must_haves:
  ...
  key_links:
    ...
    - from: "apps/coach-web/src/components/programs/program-grid/prescription-cell.tsx"
      to: "@strenly/contracts/programs/prescription"
      via: "parsePrescriptionToSeries import"
      pattern: "import.*parsePrescriptionToSeries"
---
...
interface ProgramGridProps {
  data: GridData
  programId: string
  // State management from parent - accepts SERIES ARRAY, not notation string
...
export function ProgramGrid({
  ...
  // IMPORTANT: Parse notation to series BEFORE calling parent handler
  // The parent's actions.updatePrescription expects PrescriptionSeriesInput[]
  const handlePrescriptionCommit = useCallback(
    (rowId: string, weekId: string, notation: string) => {
      // Parse the notation string into a series array
...
KEY FIX: The handlePrescriptionCommit function receives a notation string from the cell edit,
but must parse it to a series array using parsePrescriptionToSeries() before passing to
onPrescriptionChange, because useGridState.actions.updatePrescription expects series array.

2. Update the program editor page to wire everything together:

```typescript
// In $programId.tsx route
import { useGridStore, useGridData, useGridIsDirty, useGridActions } from '@/stores'
...
function ProgramEditorPage() {
  ...
  useEffect(() => {
    if (gridData && programId) {
      actions.initialize(programId, gridData)
    }
  }, [gridData, programId, actions])
  ...
  return (
    <div>
      ...
      {/* Grid - passes store actions which expect series array */}
      <ProgramGrid
        data={data}
        programId={programId}
...
Keep existing structural mutations (add/delete rows, weeks, sessions) - they can remain as immediate server operations since they affect structure, not just prescription data.
  ...
  <done>Program grid uses local state, handlePrescriptionCommit parses notation to series before calling parent, save button integrated, navigation guard active</done>
</task>
...
- [ ] program-grid.tsx uses Zustand store actions via props (not per-change mutations)
- [ ] program-grid.tsx handlePrescriptionCommit parses notation to series array
