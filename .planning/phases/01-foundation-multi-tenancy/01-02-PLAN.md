...
1. **packages/contracts/src/common/errors.ts** - Define shared error schemas:
   ...
   export const authErrors = {
     ...
     ORG_NOT_FOUND: { message: 'Organización no encontrada' },
     NOT_A_MEMBER: { message: 'No eres miembro de esta organización' },
   }
   ```

2. **packages/contracts/src/common/roles.ts** - Define role schema for validation:
   ```typescript
   import { z } from 'zod'
...
3. **packages/backend/src/lib/context.ts** - Define context types:
   ```typescript
...
4. **packages/backend/src/lib/errors.ts** - Re-export errors for procedures:
   ```typescript
   export { commonErrors, authErrors } from '@strenly/contracts/common/errors'
   ```

5. **packages/backend/src/lib/orpc.ts** - Create procedure hierarchy:
   ```typescript
...
- Role is validated with Zod schema instead of type casting
- Do NOT use `as` type casting anywhere
  </action>
  <verify>
...
- authProcedure: Validates session + organization membership with Zod-validated role
  </done>
</task>
...
- Health check uses z.literal('ok') for the status field - the type is inferred correctly
  </action>
  <verify>
...
6. Role validation uses Zod schema safeParse
</verification>
...
- Role in authProcedure validated via memberRoleSchema.safeParse (no type casting)
</success_criteria>
