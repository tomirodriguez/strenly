...
export async function seedExercises(db: DbClient): Promise<void> {
	for (const ex of EXERCISES_DATA) {
		// Insert exercise with organizationId: null for curated exercises
		await db
			.insert(exercises)
			.values({
				id: ex.id,
		...
		// Insert primary muscle mappings
		...
		// Insert secondary muscle mappings
	...
	console.log(`Seeded ${EXERCISES_DATA.length} exercises with muscle mappings`);
}
