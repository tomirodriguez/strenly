/**
 * Prescription notation parsing and formatting.
 *
 * Pure domain logic for converting between human-readable prescription
 * notation (e.g., "3x8@120kg") and structured series data.
 */

import type { IntensityType, IntensityUnit } from './types'
import { mapIntensityTypeToUnit } from './types'
 ...
 * Represents a fully parsed training prescription from notation.
 */
...
/**
 * A single series (one set) parsed from prescription notation.
 * Used as input for creating prescription series arrays.
 */
export type ParsedSeriesData = {
  orderIndex: number
  reps: number | null
  repsMax: number | null
  isAmrap: boolean
 ...
 */
export function parsePrescriptionToSeries(input: string): ParsedSeriesData[] | null {
  const trimmed = input.trim()
  ...
  const allSeries: ParsedSeriesData[] = []
  let orderIndex = 0
 ...
 *
 * @param series - Array of series data with intensity units
 * @returns Compact notation string (em dash for empty array)
 */
export function formatSeriesToNotation(series: ParsedSeriesData[]): string {
  if (series.length === 0) {
    return '—'
  }
...
/**
 * Convert domain Series array to notation string.
 * Maps intensityType to intensityUnit before formatting.
 */
export function formatDomainSeriesToNotation(
  ...
  const mapped: ParsedSeriesData[] = series.map((s, idx) => ({
    orderIndex: idx,
    reps: s.reps,
    repsMax: s.repsMax,
    isAmrap: s.isAmrap,
  ...
  return formatSeriesToNotation(mapped)
}
 ...
 */
function groupConsecutiveIdentical(series: ParsedSeriesData[]): SeriesGroup[] {
  if (series.length === 0) {
    return []
  }
...
function seriesAreIdentical(a: ParsedSeriesData, b: ParsedSeriesData): boolean {
  return (
    a.reps === b.reps &&
    a.repsMax === b.repsMax &&
    a.isAmrap === b.isAmrap &&
